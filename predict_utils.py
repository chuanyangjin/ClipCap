...
def generate2(
    ...
    embed=None,
    entry_count=1,
    entry_length=77,  # maximum number of words
    top_p=0.8,
    temperature=1.0,
    stop_token: str = ".",
):
    ...
    with torch.no_grad():
        ...
        for entry_idx in range(entry_count):
            ...
            for i in range(entry_length):
                ...
                next_token = nnf.gumbel_softmax(logits, tau = 1, hard = True).unsqueeze(1)
                next_token_embed = model.gpt.transformer.wte(next_token.argmax(dim = -1))
                if tokens is None:
                    tokens = next_token
                else:
